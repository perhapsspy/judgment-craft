...
class ValidatePluginTests(unittest.TestCase):
    ...
    def test_extra_and_missing_skill_rejected(self) -> None:
        ...
        shutil.move(skills / "judgment-craft", skills / "judgment-craft.bak")
        (skills / "extra-skill").mkdir()
        result = self.run_validator()
        self.assertEqual(result.returncode, 1)
        self.assertIn("missing skill directories: judgment-craft", result.stderr)
        self.assertIn("unexpected skill directories: extra-skill, judgment-craft.bak", result.stderr)

    def test_frontmatter_name_mismatch_rejected(self) -> None:
        path = self.fixture / "plugins" / "judgment-craft" / "skills" / "judgment-craft" / "SKILL.md"
        text = path.read_text(encoding="utf-8").replace("name: judgment-craft", "name: wrong-name")
    ...
    def test_missing_agent_metadata_rejected(self) -> None:
        path = self.fixture / "plugins" / "judgment-craft" / "skills" / "judgment-repair" / "agents" / "openai.yaml"
        path.write_text(
            'interface:\n  display_name: "Judgment Repair"\n  default_prompt: "Use $judgment-repair."\n',
            encoding="utf-8",
        )
        result = self.run_validator()
        self.assertEqual(result.returncode, 1)
        self.assertIn("judgment-repair: agents/openai.yaml interface.short_description missing", result.stderr)

    def test_starter_prompts_missing_a_skill_rejected(self) -> None:
        data = self.manifest()
        interface = data["interface"]
        assert isinstance(interface, dict)
        interface["defaultPrompt"] = ["Use $judgment-craft."]
        self.write_manifest(data)
        result = self.run_validator()
        self.assertEqual(result.returncode, 1)
        self.assertIn("manifest interface.defaultPrompt must reference $judgment-repair", result.stderr)

    def test_invalid_and_mismatched_release_tag_rejected(self) -> None:
        invalid = self.run_validator("--release-tag", "v0.2.0+build")
        self.assertEqual(invalid.returncode, 1)
        self.assertIn("must not include build metadata", invalid.stderr)

        prerelease = self.run_validator("--release-tag", "v0.2.0-rc.1")
        self.assertEqual(prerelease.returncode, 1)
        self.assertIn("must be vX.Y.Z", prerelease.stderr)

        mismatched = self.run_validator("--release-tag", "v0.2.1")
        self.assertEqual(mismatched.returncode, 1)
        self.assertIn("does not match manifest version 0.2.0", mismatched.stderr)
    ...
    def test_cli_aggregates_errors_and_exits_1(self) -> None:
        ...
        self.assertIn("--release-tag v9.9.9 does not match manifest version 0.2.0", result.stderr)
